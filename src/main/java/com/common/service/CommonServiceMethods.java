...
public class CommonServiceMethods {
    ...
    public List<GenericDropdownDTO> getCampusesByEmployee(int empId, String category) {
        ...
        if (isAdmin) {
            List<com.common.entity.Campus> adminCampuses;
            if (category != null && !category.isBlank()) {
                adminCampuses = campusRepo.findByBusinessTypeBusinessTypeNameIgnoreCaseAndIsActive(category, 1);
            } else {
                adminCampuses = campusRepo.findByIsActive(1);
            }
        ...
        if (!linkedCampusIds.isEmpty()) {
            ...
            if (category != null && !category.isBlank()) {
                linkedCampuses = campusRepo.findByCampusIdInAndBusinessTypeBusinessTypeNameIgnoreCase(linkedCampusIds,
                        category);
            } else {
                linkedCampuses = campusRepo.findByCampusIdIn(linkedCampusIds);
            }
